...
print("Processing : {}".format(model_list[-1]["model_name"]))
model, processor = load_model(model_list[-1])
model = model.to(device)

DUMMY_IMAGE = Image.fromarray(np.random.randint(255, size=(224,224,3),dtype=np.uint8))
caption = '[START-TITLE] Electroacupuncture as an adjunctive therapy for motor dysfunction in acute stroke survivors: a systematic review and meta-analyses [END-TITLE] [START-CONCEPT] Medicine , Meta-analysis , Cochrane Library , MEDLINE , Electroacupuncture , Physical therapy , Internal medicine , Systematic review , Randomized controlled trial , Publication bias , Relative risk , Stroke (engine) , Acupuncture , Rehabilitation , Confidence interval , Alternative medicine , Pathology , Mechanical engineering , Political science , Law , Engineering [END-CONCEPT] [START-SECTION] results and discussion [END-SECTION] Forest plot and meta-analysis of activities of daily living. CD, conventional drugs; EA, electroacupuncture; RT, rehabilitation therapy.'

with torch.no_grad():
    inputs = processor(text=[caption], images=[DUMMY_IMAGE], return_tensors="pt", padding=True)
